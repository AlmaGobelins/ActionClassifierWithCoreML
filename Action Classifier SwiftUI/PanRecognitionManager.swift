...
class PanRecognitionManager:ObservableObject {
    ...
    func recognizeObjectsIn(image: UIImage) -> Void {
        ...
        do {
            try predictor.makePredictions(for: image) { predictions in
                ...
                if predictionArray.contains("pan"){
                    self.isPan = true
                }
    ...
    private func formatPredictions(_ predictions: [PanPredictor.Prediction]) -> [String] {
